    ...
    <section className="px-8 md:px-12 py-32">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-16" data-reveal>
          ...
          {showViewAll && (
            <Link to="/projects" className="text-meta border-b border-[#C8A45D] text-[#C8A45D] pb-2 hover:text-white hover:border-white w-fit">
              View all projects →
            </Link>
          )}
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((project) => (
            <article key={project.slug} data-reveal className="group relative aspect-[4/5] overflow-hidden bg-black">
              <img
                src={project.img}
                alt={project.client}
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                ...
                <p className="text-meta mt-1 text-white/70">{project.location}</p>
                <Link
                  to="/projects"
